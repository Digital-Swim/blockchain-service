...
async function createOrdinalInscription({
    ...
    try {
        ...
        // 1. Derive Keys
        const keyPair = ecPair.fromWIF(privateKeyWIF, networks.testnet);
        const internalPubkey = keyPair.publicKey.slice(1); // remove first byte for internal pubkey
        const p2tr = bitcoin.payments.p2tr({ Buffer.from(internalPubkey), network: networks.testnet });

        // 2. Create Inscription Script (TapScript leaf)
        const inscriptionScript = script.compile([
            script.OPS.OP_FALSE,
            script.OPS.OP_IF,
        ...
        const commitPsbt = new Psbt({ network: networks.testnet });
        ...
        if (changeAmount >= 546) {
            ...
            const p2trChange = payments.p2tr({ internalPubkey, network: networks.testnet });
            commitPsbt.addOutput({
        ...
        const revealPsbt = new Psbt({ network: networks.testnet });
