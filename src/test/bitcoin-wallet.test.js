...
import { BitcoinWallet } from '../wallets/bitcoin/wallet.js';
import { BitcoinAddress } from '../wallets/bitcoin/address.js';
import { verifyMessage } from '../utils/common.js';
const supportedNetworks = ['mainnet', 'testnet', 'regtest'];
const networks = {
    ...
    for (const networkName of supportedNetworks) {
        ...
        describe(`Network: ${networkName}`, () => {
            ...
            it('should throw on unsupported address type', () => {
                expect(() => address.getAddress('foo')).to.throw('Unsupported address type');
            });
        });
    }
});
